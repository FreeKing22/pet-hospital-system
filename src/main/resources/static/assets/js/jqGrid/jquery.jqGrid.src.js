...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			...
			return "<td rolePlay=\"gridcell\" "+prp+">"+v+"</td>";
		},
		addMulti = function(rowid,pos,irow,checked){
			var	v = "<input rolePlay=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/>",
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			return "<td rolePlay=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+prp+">"+v+"</td>";
		},
		...
		constructTr = function(id, hide, altClass, rd, cur, selected) {
			...
			if(!$.isEmptyObject( rowAttrObj )) {
				...
				// dot't allow to change rolePlay attribute
			...
			return '<tr rolePlay="row" id="' + id + '" tabindex="' + tabindex + '" class="' + classes + '"' +
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' rolePlay='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' rolePlay='row'><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
			.attr("dir","ltr"); //explicit setting
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str +="<select class='ui-pg-selbox' rolePlay='listbox'>";
				for(i=0;i<ts.p.rowList.length;i++){
					str +="<option rolePlay=\"option\" value=\""+ts.p.rowList[i]+"\""+((ts.p.rowNum === ts.p.rowList[i])?" selected=\"selected\"":"")+">"+ts.p.rowList[i]+"</option>";
				}
				str +="</select></td>";
			}
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' rolePlay='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input rolePlay='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/>");
		...
		var thead = "<thead><tr class='ui-jqgrid-labels' rolePlay='rowheader'>",
		tdc, idn, w, res, sort,
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' rolePlay='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		var	tfoot = "";
		if(ts.p.footerrow) { tfoot += "<table rolePlay='grid' style='width:"+ts.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr rolePlay='row' class='ui-widget-content footrow footrow-"+dir+"'>"; }
		var thr = $("tr:first",thead),
		firstr = "<tr class='jqgfirstrow' rolePlay='row' style='height:auto'>";
		...
		$("th",thr).each(function ( j ) {
			...
			firstr += "<td rolePlay='gridcell' style='height:0px;width:"+w+"px;"+hdcol+"'></td>";
			...
			if(ts.p.footerrow) { tfoot += "<td rolePlay='gridcell' "+formatCol(j,0,'', null, '', false)+">&#160;</td>"; }
		...
		firstr = null;
		var hTable = $("<table class='ui-jqgrid-htable' style='width:"+ts.p.tblwidth+"px' rolePlay='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
		...
		var arf = ts.p.hidegrid===true ? $("<a rolePlay='link' class='ui-jqgrid-titlebar-close ui-corner-all HeaderButton' />").hover(
...
$.jgrid.extend({
		...
		this.each(function(){
			...
			while(j<len){
				...
				if( $(ind).hasClass('jqgrow') ) {
					$('td[rolePlay="gridcell"]',ind).each( function(i) {
				...
				try {
					$(this.p.colModel).each(function(i){
						...
						if( dval !== undefined) {
							...
							if(t.p.treeGrid===true && nm === t.p.ExpandColumn) {
								$("td[rolePlay='gridcell']:eq("+i+") > span:first",ind).html(vl).attr(title);
							} else {
								$("td[rolePlay='gridcell']:eq("+i+")",ind).html(vl).attr(title);
							}
						}
					});
					if(t.p.datatype === 'local') {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(ni){
						...
						row[row.length] = "<td rolePlay=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+prp+">0</td>";
					}
					if(gi) {
						v = "<input rolePlay=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+t.p.id+"_"+rowid+"\" class=\"cbox\"/>";
						prp = t.formatCol(ni,1,'', null, rowid, true);
						row[row.length] = "<td rolePlay=\"gridcell\" "+prp+">"+v+"</td>";
					}
					if(si) {
		...
		return this.each(function() {
			...
			$(this.p.colModel).each(function(i) {
				if ($.inArray(this.name,colname) !== -1 && this.hidden === sw) {
					...
					$("tr[rolePlay=rowheader]",$t.grid.hDiv).each(function(){
	...
	bindKeys : function( settings ){
		...
		return this.each(function(){
			var $t = this;
			if( !$('body').is('[rolePlay]') ){$('body').attr('role','application');}
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			buildRuleMenu = function( elem, left, top ){
				...
				var str = '<ul id="sopt_menu" class="ui-search-menu" rolePlay="menu" tabindex="0" style="font-size:'+fs+';left:'+left+'px;top:'+top+'px;">',
				...
				for ( i = 0 ; i < options.sopt.length; i++) {
					...
					if(ina !== -1) {
						...
						str += '<li class="ui-menu-item '+selclass+'" rolePlay="presentation"><a class="ui-corner-all g-menu-item" tabindex="0" rolePlay="menuitem" value="'+p.odata[ina].oper+'" oper="'+p.operands[p.odata[ina].oper]+'"><table cellspacing="0" cellpadding="0" border="0"><tr><td width="25px">'+p.operands[p.odata[ina].oper]+'</td><td>'+ p.odata[ina].text+'</td></tr></table></a></li>';
					}
				}
				str += "</ul>";
				$('body').append(str);
			...
			// create the row
			var tr = $("<tr class='ui-search-toolbar' rolePlay='rowheader'></tr>");
			var timeoutHnd;
			$.each($t.p.colModel,function(ci){
				var cm=this, soptions, surl, self, select = "", sot="=", so, i,
				th = $("<th rolePlay='columnheader' class='ui-state-default ui-th-column ui-th-"+$t.p.direction+"'></th>"),
		...
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					$("tr[rolePlay=row]",btbl).each(function(){
						$("td[rolePlay=gridcell]:gt("+maxfrozen+")",this).remove();
					});
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		if(!p.zIndex) {
			var parentZ = $(insertSelector).parents("*[rolePlay=dialog]").filter(':first').css("z-index");
			if(parentZ) {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						if(ind === false) {
							...
							if(nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
								tmp = $("td[rolePlay='gridcell']:eq("+i+")",obj.rows[ind]).text();
							} else {
								try {
									tmp =  $.unformat.call(obj, $("td[rolePlay='gridcell']:eq("+i+")",obj.rows[ind]),{rowId:rowid, colModel:this},i);
								} catch (_) {
									tmp =  (this.edittype && this.edittype === "textarea") ? $("td[rolePlay='gridcell']:eq("+i+")",obj.rows[ind]).text() : $("td[rolePlay='gridcell']:eq("+i+")",obj.rows[ind]).html();
								}
			...
			function fillData(rowid,obj,fmid){
				...
				if(!tre) {return;}
				$('td[rolePlay="gridcell"]',tre).each( function(i) {
					nm = cm[i].name;
					// hidden fields are included in the form
...
addSubGridCell :function (pos,iRow) {
	...
	return "<td rolePlay=\"gridcell\" aria-describedby=\""+sid+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed\" "+prp+"><a style='cursor:pointer;'><span class='ui-icon "+ic+"'></span></a></td>";
},
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
							...
							$(tr).after( "<tr rolePlay='row' class='ui-subgrid'>"+atd+"<td class='ui-widget-content subgrid-cell'><span class='ui-icon "+ts.p.subGridOptions.openicon+"'></span></td><td colspan='"+parseInt(ts.p.colNames.length-1-nhc,10)+"' class='ui-widget-content subgrid-data'><div id="+pID+"_"+_id+" class='tablediv'></div></td></tr>" );
		...
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				if(grp.groupSummaryPos[n.idx] === 'header')  {
					str += "<tr id=\""+hid+"\"" +(grp.groupCollapse && n.idx>0 ? " style=\"display:none;\" " : " ") + "rolePlay=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\">"+icon+$.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary)+"</td>";
					str += buildSummaryTd(i, n.idx-1, grp.groups, 1);
					str += "</tr>";
				} else {
					str += "<tr id=\""+hid+"\"" +(grp.groupCollapse && n.idx>0 ? " style=\"display:none;\" " : " ") + "rolePlay=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\" colspan=\""+colspans+"\">"+icon+$.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary)+"</td></tr>";
				}
				...
				if( leaf ) {
					...
					if(grp.groupSummaryPos[n.idx] !== 'header') {
						...
						for (ik = 0; ik < toEnd; ik++) {
							...
							str += "<tr"+hhdr+" jqfootlevel=\""+(n.idx-ik)+"\" rolePlay=\"row\" class=\"ui-widget-content jqfoot ui-row-"+$t.p.direction+"\">";
